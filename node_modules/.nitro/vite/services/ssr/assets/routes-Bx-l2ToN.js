import { i as apiFetch } from "./useAuth-COIZ1r_K.js";
import { a as useHeroBanners, c as useStoreMetadata, d as AnnouncementBar, f as TopBar, i as useCategories, l as StoreHeader, n as NavBar, r as Skeleton, s as useProducts, t as StoreFooter } from "./StoreFooter-DMLW-oTe.js";
import { t as ProductCard } from "./ProductCard-aFJKB8mL.js";
import { useCallback, useEffect, useState } from "react";
import { Link } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import { BatteryCharging, CircuitBoard, Printer, Zap } from "lucide-react";
import useEmblaCarousel from "embla-carousel-react";
//#region src/components/store/HeroSection.tsx
var serviceIcons = {
	"PCB Manufacturing": CircuitBoard,
	"3D Printing": Printer,
	"Laser Cutting": Zap,
	"Custom Battery Pack": BatteryCharging
};
var serviceToCategory = {
	"PCB Manufacturing": "Development Boards",
	"3D Printing": "3D Printers and Parts",
	"Laser Cutting": "DIY and Maker Kits",
	"Custom Battery Pack": "Batteries, Power Supply and Accessories"
};
function HeroSection() {
	const { data: banners, loading: bannersLoading, error: bannersError } = useHeroBanners();
	const { data: meta, loading: metaLoading, error: metadataError } = useStoreMetadata();
	const [emblaRef, emblaApi] = useEmblaCarousel({ loop: true });
	const [selected, setSelected] = useState(0);
	const onSelect = useCallback(() => {
		if (emblaApi) setSelected(emblaApi.selectedScrollSnap());
	}, [emblaApi]);
	useEffect(() => {
		if (!emblaApi) return;
		emblaApi.on("select", onSelect);
		const timer = setInterval(() => emblaApi.scrollNext(), 5e3);
		return () => {
			clearInterval(timer);
			emblaApi.off("select", onSelect);
		};
	}, [emblaApi, onSelect]);
	if (bannersLoading || metaLoading) return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-6",
		children: /* @__PURE__ */ jsxs("div", {
			className: "grid gap-4 lg:grid-cols-[240px_1fr]",
			children: [/* @__PURE__ */ jsx("div", {
				className: "hidden flex-col gap-3 lg:flex",
				children: Array.from({ length: 4 }).map((_, i) => /* @__PURE__ */ jsx(Skeleton, { className: "h-[73px] w-full rounded-lg" }, i))
			}), /* @__PURE__ */ jsx(Skeleton, { className: "h-[200px] sm:h-[300px] lg:h-[320px] w-full rounded-xl" })]
		})
	});
	if (bannersError || metadataError || !banners?.length || !meta) return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-6",
		children: /* @__PURE__ */ jsxs("div", {
			className: "rounded-xl border border-dashed border-border bg-card px-6 py-12 text-center",
			children: [/* @__PURE__ */ jsx("h2", {
				className: "text-xl font-bold text-foreground",
				children: "Homepage content is temporarily unavailable"
			}), /* @__PURE__ */ jsx("p", {
				className: "mt-2 text-sm text-muted-foreground",
				children: "Please refresh the page to try again."
			})]
		})
	});
	return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-6",
		children: /* @__PURE__ */ jsxs("div", {
			className: "grid gap-4 lg:grid-cols-[240px_1fr]",
			children: [/* @__PURE__ */ jsx("aside", {
				className: "hidden flex-col gap-2 lg:flex",
				children: meta.services.map((s, idx) => {
					const Icon = serviceIcons[s.name] || CircuitBoard;
					const tint = idx % 2 === 0 ? "bg-accent/10" : "bg-primary/10";
					const category = serviceToCategory[s.name];
					return /* @__PURE__ */ jsxs(Link, {
						to: "/shop",
						search: { category },
						className: `flex flex-1 items-center justify-between rounded-lg ${tint} px-4 py-3 transition-transform hover:-translate-y-0.5`,
						children: [/* @__PURE__ */ jsx("span", {
							className: "text-sm font-semibold text-foreground",
							children: s.name
						}), /* @__PURE__ */ jsx(Icon, { className: "h-8 w-8 text-primary" })]
					}, s.name);
				})
			}), /* @__PURE__ */ jsxs("div", {
				className: "relative overflow-hidden rounded-xl bg-secondary/20",
				children: [/* @__PURE__ */ jsx("div", {
					ref: emblaRef,
					className: "overflow-hidden",
					children: /* @__PURE__ */ jsx("div", {
						className: "flex",
						children: banners.map((banner, i) => /* @__PURE__ */ jsx("div", {
							className: "min-w-0 flex-[0_0_100%] aspect-[3/1]",
							children: /* @__PURE__ */ jsx("img", {
								src: banner.image,
								alt: banner.alt,
								width: 1920,
								height: 512,
								loading: i === 0 ? "eager" : "lazy",
								onError: (event) => {
									event.currentTarget.onerror = null;
									event.currentTarget.src = "/assets/hero-1.jpg";
								},
								className: "h-full w-full object-cover"
							})
						}, i))
					})
				}), /* @__PURE__ */ jsx("div", {
					className: "absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5",
					children: banners.map((_, i) => /* @__PURE__ */ jsx("button", {
						"aria-label": `Go to slide ${i + 1}`,
						onClick: () => emblaApi?.scrollTo(i),
						className: `h-1.5 rounded-full transition-all cursor-pointer ${selected === i ? "w-5 bg-primary" : "w-1.5 bg-primary/40"}`
					}, i))
				})]
			})]
		})
	});
}
//#endregion
//#region src/components/store/CategoryGrid.tsx
function CategoryGrid() {
	const { data: categoriesList, loading, error } = useCategories();
	if (loading) return /* @__PURE__ */ jsxs("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "mb-6 flex items-center justify-between gap-4",
			children: [/* @__PURE__ */ jsxs("div", {
				className: "flex flex-1 items-center gap-4",
				children: [/* @__PURE__ */ jsx(Skeleton, { className: "h-8 w-44" }), /* @__PURE__ */ jsx(Skeleton, { className: "h-px flex-1" })]
			}), /* @__PURE__ */ jsx(Skeleton, { className: "h-9 w-24" })]
		}), /* @__PURE__ */ jsx("div", {
			className: "grid grid-cols-2 gap-x-5 gap-y-8 sm:grid-cols-3 lg:grid-cols-6",
			children: Array.from({ length: 6 }).map((_, i) => /* @__PURE__ */ jsxs("div", {
				className: "space-y-3",
				children: [/* @__PURE__ */ jsx(Skeleton, { className: "aspect-square w-full rounded-lg" }), /* @__PURE__ */ jsx(Skeleton, { className: "h-4 w-3/4" })]
			}, i))
		})]
	});
	if (error) return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: /* @__PURE__ */ jsxs("div", {
			className: "rounded-lg border border-dashed border-border bg-card px-5 py-8 text-center",
			children: [/* @__PURE__ */ jsx("h2", {
				className: "text-lg font-bold text-foreground",
				children: "Categories are temporarily unavailable"
			}), /* @__PURE__ */ jsx("p", {
				className: "mt-1 text-sm text-muted-foreground",
				children: "Please refresh the page to try again."
			})]
		})
	});
	if (!categoriesList?.length) return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: /* @__PURE__ */ jsxs("div", {
			className: "rounded-lg border border-dashed border-border bg-card px-5 py-8 text-center",
			children: [
				/* @__PURE__ */ jsx("h2", {
					className: "text-lg font-bold text-foreground",
					children: "Categories are being updated"
				}),
				/* @__PURE__ */ jsx("p", {
					className: "mt-1 text-sm text-muted-foreground",
					children: "Browse the full catalog while new categories are added."
				}),
				/* @__PURE__ */ jsx(Link, {
					to: "/shop",
					className: "mt-4 inline-flex rounded-lg border border-primary px-4 py-2 text-sm font-semibold text-primary hover:bg-primary hover:text-primary-foreground",
					children: "Browse products"
				})
			]
		})
	});
	return /* @__PURE__ */ jsxs("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "mb-6 flex items-center justify-between gap-4",
			children: [/* @__PURE__ */ jsxs("div", {
				className: "flex flex-1 items-center gap-4",
				children: [/* @__PURE__ */ jsx("h2", {
					className: "text-2xl font-bold text-foreground md:text-3xl",
					children: "Categories"
				}), /* @__PURE__ */ jsx("div", { className: "h-px flex-1 bg-border" })]
			}), /* @__PURE__ */ jsx(Link, {
				to: "/shop",
				className: "rounded-lg border border-primary px-5 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground",
				children: "View All"
			})]
		}), /* @__PURE__ */ jsx("div", {
			className: "grid grid-cols-2 gap-x-5 gap-y-8 sm:grid-cols-3 lg:grid-cols-6",
			children: categoriesList.map((cat) => /* @__PURE__ */ jsxs(Link, {
				to: "/shop",
				search: { category: cat.name },
				className: "group",
				children: [/* @__PURE__ */ jsx("div", {
					className: "overflow-hidden rounded-lg",
					children: /* @__PURE__ */ jsx("img", {
						src: typeof cat.image === "string" ? cat.image : cat.image?.url,
						alt: cat.name,
						width: 640,
						height: 640,
						loading: "lazy",
						onError: (event) => {
							event.currentTarget.onerror = null;
							event.currentTarget.src = "/assets/cat-components.jpg";
						},
						className: "aspect-square w-full object-cover transition-transform duration-300 group-hover:scale-105"
					})
				}), /* @__PURE__ */ jsx("h3", {
					className: "mt-3 text-sm font-semibold leading-snug text-foreground group-hover:text-primary",
					children: cat.name
				})]
			}, cat.name))
		})]
	});
}
//#endregion
//#region src/components/store/FeaturedProducts.tsx
function FeaturedProducts() {
	const { data: products, loading, error } = useProducts();
	if (loading) return /* @__PURE__ */ jsxs("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "mb-6 flex items-center justify-between gap-4",
			children: [/* @__PURE__ */ jsxs("div", {
				className: "flex flex-1 items-center gap-4",
				children: [/* @__PURE__ */ jsx(Skeleton, { className: "h-8 w-52" }), /* @__PURE__ */ jsx(Skeleton, { className: "h-px flex-1" })]
			}), /* @__PURE__ */ jsx(Skeleton, { className: "h-9 w-24" })]
		}), /* @__PURE__ */ jsx("div", {
			className: "grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6",
			children: Array.from({ length: 6 }).map((_, i) => /* @__PURE__ */ jsxs("div", {
				className: "border border-border rounded-lg p-3 space-y-3",
				children: [
					/* @__PURE__ */ jsx(Skeleton, { className: "h-3 w-12" }),
					/* @__PURE__ */ jsx(Skeleton, { className: "aspect-square w-full rounded" }),
					/* @__PURE__ */ jsx(Skeleton, { className: "h-4 w-3/4" }),
					/* @__PURE__ */ jsx(Skeleton, { className: "h-3 w-16" }),
					/* @__PURE__ */ jsx(Skeleton, { className: "h-7 w-full" })
				]
			}, i))
		})]
	});
	if (error) return /* @__PURE__ */ jsx("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: /* @__PURE__ */ jsxs("div", {
			className: "rounded-lg border border-dashed border-border bg-card px-5 py-8 text-center",
			children: [/* @__PURE__ */ jsx("h2", {
				className: "text-lg font-bold text-foreground",
				children: "Featured products are temporarily unavailable"
			}), /* @__PURE__ */ jsx("p", {
				className: "mt-1 text-sm text-muted-foreground",
				children: "Please refresh the page to try again."
			})]
		})
	});
	const displayProducts = (products ?? []).filter((product) => product.featured).slice(0, 6);
	if (!displayProducts.length) return null;
	return /* @__PURE__ */ jsxs("section", {
		className: "mx-auto max-w-7xl px-4 py-8",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "mb-6 flex items-center justify-between gap-4",
			children: [/* @__PURE__ */ jsxs("div", {
				className: "flex flex-1 items-center gap-4",
				children: [/* @__PURE__ */ jsx("h2", {
					className: "text-2xl font-bold text-foreground md:text-3xl",
					children: "Featured Products"
				}), /* @__PURE__ */ jsx("div", { className: "h-px flex-1 bg-border" })]
			}), /* @__PURE__ */ jsx(Link, {
				to: "/shop",
				className: "rounded-lg border border-primary px-5 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground",
				children: "View All"
			})]
		}), /* @__PURE__ */ jsx("div", {
			className: "grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6",
			children: displayProducts.map((product) => /* @__PURE__ */ jsx(ProductCard, { product }, product.sku))
		})]
	});
}
//#endregion
//#region src/components/store/TrustedBrands.tsx
var api = "http://localhost:4000".replace(/\/$/, "") ?? "";
function TrustedBrands() {
	const [brands, setBrands] = useState([]);
	useEffect(() => {
		apiFetch("/api/brands").then((rows) => setBrands(rows.filter((b) => b.active))).catch(() => setBrands([]));
	}, []);
	if (!brands.length) return null;
	return /* @__PURE__ */ jsxs("section", {
		className: "mx-auto max-w-7xl px-4 py-10",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "text-center",
			children: [/* @__PURE__ */ jsx("p", {
				className: "text-sm font-bold tracking-wide text-accent",
				children: "OUR PARTNERS"
			}), /* @__PURE__ */ jsx("h2", {
				className: "mt-1 text-2xl font-extrabold text-foreground",
				children: "Trusted Brands"
			})]
		}), /* @__PURE__ */ jsx("div", {
			className: "mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7",
			children: brands.map((brand) => /* @__PURE__ */ jsx("div", {
				className: "flex aspect-[2/1] items-center justify-center rounded-lg border bg-card p-4",
				children: brand.logoUrl ? /* @__PURE__ */ jsx("img", {
					src: brand.logoUrl.startsWith("http") ? brand.logoUrl : `${api}${brand.logoUrl}`,
					alt: brand.name,
					onError: (event) => {
						event.currentTarget.onerror = null;
						event.currentTarget.style.display = "none";
					},
					className: "max-h-full max-w-full object-contain"
				}) : /* @__PURE__ */ jsx("span", {
					className: "text-sm font-semibold text-muted-foreground",
					children: brand.name
				})
			}, brand.id))
		})]
	});
}
//#endregion
//#region src/routes/index.tsx?tsr-split=component
function Index() {
	return /* @__PURE__ */ jsxs("div", {
		className: "min-h-screen bg-background",
		children: [
			/* @__PURE__ */ jsx(AnnouncementBar, {}),
			/* @__PURE__ */ jsx(TopBar, {}),
			/* @__PURE__ */ jsx(StoreHeader, {}),
			/* @__PURE__ */ jsx(NavBar, {}),
			/* @__PURE__ */ jsxs("main", { children: [
				/* @__PURE__ */ jsx("h1", {
					className: "sr-only",
					children: "AutoForge Robotics — Robotics, Electronics & Maker Components Store"
				}),
				/* @__PURE__ */ jsx(HeroSection, {}),
				/* @__PURE__ */ jsx(CategoryGrid, {}),
				/* @__PURE__ */ jsx(FeaturedProducts, {}),
				/* @__PURE__ */ jsx(TrustedBrands, {})
			] }),
			/* @__PURE__ */ jsx(StoreFooter, {})
		]
	});
}
//#endregion
export { Index as component };
