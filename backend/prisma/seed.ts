...
async function main() {
  ...
  if (!(await prisma.inventoryMovement.count({where:{productId:product.id}}))) await prisma.inventoryMovement.create({data:{productId:product.id,delta:12,quantityAfter:12,action:"RECEIVED",note:"Initial seed inventory"}});
  for (const [sortOrder,name] of ["Panasonic","SafeConnect","OpenMV","Teensy","Benewake","Samsung","Indian Army"].entries()) await prisma.brandCollaboration.upsert({where:{name},update:{sortOrder},create:{name,logoUrl:"/uploads/placeholder-brand.svg",sortOrder}});
  ...
  if (!(await prisma.enquiry.count({ where: { type: "sell" } }))) await prisma.enquiry.create({data:{type:"sell",name:"Priya Sharma",companyName:"Maker Labs",email:"priya@example.com",phone:"9876543210",message:"Interested in listing electronics components.",details:{businessType:"Distributor",city:"Pune",country:"India"}}});
}
