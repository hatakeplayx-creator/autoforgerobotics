...
app.post("/api/products", auth, admin, asyncRoute(async (req, res) => { const d = productSchema.parse(req.body); const { imageIds, ...values } = d; const productData = { ...values, specifications: values.specifications }; const p = await prisma.$transaction(async (tx) => { const product = await tx.product.create({ data: productData }); if (imageIds?.length)
    await tx.productImage.createMany({ data: imageIds.map((mediaId, sortOrder) => ({ productId: product.id, mediaId, sortOrder })) }); if (d.stockQuantity)
    await tx.inventoryMovement.create({ data: { productId: product.id, delta: d.stockQuantity, quantityAfter: d.stockQuantity, action: InventoryAction.RECEIVED, note: "Initial stock" } }); return product; }); res.status(201).json(p); }));
app.patch("/api/products/:id", auth, admin, asyncRoute(async (req, res) => { const d = productSchema.partial().parse(req.body); const { imageIds, ...values } = d; const productData = { ...values, specifications: values.specifications }; const id = idSchema.parse(req.params).id; const p = await prisma.$transaction(async (tx) => { const product = await tx.product.update({ where: { id }, data: productData }); if (imageIds) {
    await tx.productImage.deleteMany({ where: { productId: id } });
    if (imageIds.length)
        await tx.productImage.createMany({ data: imageIds.map((mediaId, sortOrder) => ({ productId: id, mediaId, sortOrder })) });
} return product; }); res.json(p); }));
...
const parseImportRows = (value) => z.object({ rows: z.array(z.unknown()).min(1).max(500) }).parse(value).rows.map((row, index) => { const parsed = importRowSchema.safeParse(row); return parsed.success ? { row: index + 1, data: parsed.data } : { row: index + 1, errors: parsed.error.issues.map(issue => `${issue.path.join(".") || "row"}: ${issue.message}`) }; });
app.post("/api/products/import/preview", auth, admin, asyncRoute(async (req, res) => { const parsed = parseImportRows(req.body); res.json({ valid: parsed.filter((entry) => "data" in entry).map(entry => entry.data), invalid: parsed.filter((entry) => "errors" in entry) }); }));
app.post("/api/products/import", auth, admin, asyncRoute(async (req, res) => { const input = z.object({ rows: z.array(z.unknown()).min(1).max(500), dryRun: z.boolean().default(false) }).parse(req.body); const parsed = parseImportRows({ rows: input.rows }); const invalid = parsed.filter((entry) => "errors" in entry); const valid = parsed.filter((entry) => "data" in entry); const batchId = randomBytes(12).toString("hex"); if (invalid.length) {
    res.status(422).json({ message: "Import validation failed", batchId, created: 0, updated: 0, categories: 0, invalid });
    return;
} if (input.dryRun) {
    res.json({ batchId, created: 0, updated: 0, categories: [...new Set(valid.map(entry => entry.data.category).filter(Boolean))].length, invalid: [] });
    return;
} const result = await prisma.$transaction(async (tx) => { let created = 0, updated = 0, categories = 0; for (const { data } of valid) {
    let categoryId;
    if (data.category) {
        let category = await tx.category.findFirst({ where: { name: { equals: data.category, mode: "insensitive" } } });
        if (!category) {
            category = await tx.category.create({ data: { name: data.category, slug: slugify(data.category) } });
            categories++;
        }
    ...
    for (const url of data.imageUrls ?? []) {
        const key = `import-${createHash("sha256").update(url).digest("hex")}`;
        const filename = path.basename(new URL(url).pathname) || "imported-image";
        const media = await tx.media.upsert({ where: { key }, update: { url, filename }, create: { key, url, filename, mimeType: "image/*", size: 0 } });
        imageIds.push(media.id);
    }
    const productData = { name: data.name, slug: data.slug ?? slugify(data.name), description: data.description, price: data.price, compareAtPrice: data.compareAtPrice, stockQuantity: data.stockQuantity, lowStockThreshold: data.lowStockThreshold, featured: data.featured, brand: data.brand, categoryId, specifications: data.specifications };
    const product = existing ? await tx.product.update({ where: { id: existing.id }, data: productData }) : await tx.product.create({ data: { ...productData, sku: data.sku } });
    if (existing)
        updated++;
    else {
        created++;
        if (data.stockQuantity)
            await tx.inventoryMovement.create({ data: { productId: product.id, delta: data.stockQuantity, quantityAfter: data.stockQuantity, action: InventoryAction.RECEIVED, note: `Import ${batchId}` } });
    }
    await tx.productImage.deleteMany({ where: { productId: product.id } });
    if (imageIds.length)
        await tx.productImage.createMany({ data: imageIds.map((mediaId, sortOrder) => ({ productId: product.id, mediaId, sortOrder })) });
} return { created, updated, categories }; }); res.json({ batchId, ...result, invalid: [] }); }));
...
app.get("/api/ready", asyncRoute(async (_req, res) => { try {
...
app.put("/api/settings/:key", auth, admin, asyncRoute(async (req, res) => { const { value } = settingWriteSchema.parse(req.body); const key = z.string().min(1).max(120).parse(req.params.key); res.json(await prisma.setting.upsert({ where: { key }, update: { value: value }, create: { key, value: value } })); }));
