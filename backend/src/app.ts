...
app.use(helmet({crossOriginResourcePolicy:{policy:"cross-origin"}})); app.use(cors({origin:env.CORS_ORIGIN.split(","),credentials:false})); app.use(express.json({limit:"1mb"})); app.use(rateLimit({windowMs:15*60e3,max:300,standardHeaders:true,legacyHeaders:false})); app.use("/uploads",express.static(uploadDir,{maxAge:"1y",immutable:true}));
...
app.post("/api/orders",auth,asyncRoute(async(req,res)=>{
  ...
  const order=await prisma.$transaction(async tx=>{
    ...
    for(const line of input.items){
      ...
      subtotal+=itemTotal;tax+=itemTax;
      lines.push({
...
interface ShippingProvider {
  ...
  getTracking(awbNumber: string): Promise<any>;
}

class MockShippingProvider implements ShippingProvider {
  async createShipment(order: any){ return {awbNumber:`AWB-${Date.now()}`,trackingNumber:`TRK-${Date.now()}`}; }
  async getTracking(awbNumber: string){ return {status:"IN_TRANSIT"}; }
}
...
app.get("/api/settings",asyncRoute(async(_req,res)=>res.json(await prisma.setting.findMany())));app.put("/api/settings/:key",auth,admin,asyncRoute(async(req,res)=>{const value=z.unknown().parse(req.body.value);const key=String(req.params.key);res.json(await prisma.setting.upsert({where:{key},update:{value:value as any},create:{key,value:value as any}}));}));
