...
import { t as WishlistContext } from "./WishlistContext-YfAgkqnk.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/StoreFooter-DMLW-oTe.js
...
var homepageBlocksRequest = null;
...
async function getCategories() {
	if (!categoriesRequest) categoriesRequest = apiFetch("/api/categories").then((categories) => categories.map((category) => ({
		...category,
...
async function getBlocks() {
	if (!homepageBlocksRequest) homepageBlocksRequest = apiFetch("/api/homepage").catch((error) => {
...
function useProducts(filters) {
	const deferredFilters = (0, import_react.useDeferredValue)(filters);
	...
	(0, import_react.useEffect)(() => {
		...
		getProducts(deferredFilters).then((products) => {
			if (!active) return;
			setState({
				data: products,
				loading: false,
...
//#endregion
export { StoreHeader as a, useHeroBanners as c, useStoreMetadata as d, useWishlist as f, StoreFooter as i, useProductDetails as l, NavBar as n, TopBar as o, Skeleton as r, useCategories as s, AnnouncementBar as t, useProducts as u };
